...
function Bar({ label, value, max, color }) {
  ...
  return (
    <div className="flex items-center gap-4">
      <span className="w-36 text-sm font-mono text-surface-200/70 truncate">
        {label}
      </span>
      ...
      <span className="w-24 text-right text-sm tabular-nums text-surface-200/60">
...
export default function CostBreakdown() {
  ...
  if (!data)
    return (
      <div className="p-8 text-surface-200/40">Failed to load analytics.</div>
    );
  ...
  return (
    <div className="p-8 animate-fade-in">
      ...
      <p className="text-sm text-surface-200/60 mb-8">
        Aggregated cost and token usage across all traces.
      </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        ...
        <StatCard
          ...
          value={
            data.avg_cost_per_trace < 0.01
              ? `$${data.avg_cost_per_trace.toFixed(4)}`
              : `$${data.avg_cost_per_trace.toFixed(2)}`
          }
          icon={
            <svg
              className="w-5 h-5"
            ...
            {costEntries
              ...
              .map(([model, cost]) => (
                <Bar
                  ...
                  value={cost}
                  max={maxCost}
                  color="bg-gradient-to-r from-accent-dark to-accent-light"
                />
              ))}
          </div>
        </div>
      ...
      {data.total_traces === 0 && (
        <div className="text-center text-surface-200/40 py-16">
          ...
          <p className="text-sm mt-1">
            Start tracing agent runs with the SDK to see cost analytics here.
          </p>
        </div>
      )}
          ...
          <div className="bg-surface-800/40 rounded-xl border border-white/5 p-6">
            <div className="flex items-center gap-6 mb-3">
              <span className="flex items-center gap-2 text-sm">
                <span className="w-3 h-3 rounded bg-accent" /> Prompt (
                {data.total_prompt_tokens.toLocaleString()})
              </span>
              <span className="flex items-center gap-2 text-sm">
                <span className="w-3 h-3 rounded bg-emerald-400" /> Completion (
            ...
            <div className="h-4 rounded-full overflow-hidden flex bg-surface-900/60">
              <div
                ...
                style={{
                  width: `${(data.total_prompt_tokens / data.total_tokens) * 100}%`,
                }}
              />
              <div
                className="bg-emerald-400 h-full transition-all duration-700"
                style={{
                  width: `${(data.total_completion_tokens / data.total_tokens) * 100}%`,
                }}
              />
            </div>
          </div>
